...
const EmbedColorPill = ({ color }) => {
  ...
  const style = { backgroundColor: color ? color : "black" };
...
const EmbedField = ({ name, value, inline, id }) => {
  if (!name && !value) {
    return null;
  }
  ...
  const fieldValue = value ? (<div className='embed-field-value markup' style={{color: "white"}}>{parseAllowLinks(value)}</div>) : null;

  return <div className={cls} inline={inline}>{fieldName}{fieldValue}</div>;
};
...
const EmbedImage = ({ url }) => {
  ...
  return <a className='embed-thumbnail embed-thumbnail-rich'><img className='image' role='presentation' src={url} style={{width: "280px"}}/></a>;
};
